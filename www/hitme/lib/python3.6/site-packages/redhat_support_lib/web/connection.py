...
class Connection(object):
    ...
    def _doUpload(self, url, fileName, fileChunk=None, description=None):
        ...
        '''
        def upload_progress(fh, file_size):
            fh_tell = fh.tell()
            percent = str(int(fh_tell * 100 / file_size))
            logger.debug('upload_progress: fh.tell() = %d, file_size = %d, percent = %s'
                         % (fh_tell, file_size, percent))
        ...
        try:
            try:
                ...
                # Setup thread to display upload progress
                logger.debug('Starting TimerThread ...')
                timer = TimerThread(1, upload_progress, fh, fileSize)
                ...
                while True:
                    ...
                    if fileChunk:
                        ...
                        chunk_size = fileChunk['size']
                        if fileSize - fh.tell() < chunk_size:
                            chunk_size = fileSize - fh.tell()
                        chunk_end = fh.tell() + chunk_size
                    else:
                    ...
                    if fileChunk:
                        while fh.tell() < chunk_end:
                            self.__connection.send(fh.read(self.read_size))
...
# idle-ttl expiration, rather than after the call
        ...
        def makeHTTPConnection():
            ...
            if self.proxy_url:
                ...
                if self.proxy_user and self.proxy_pass:
                    # BZ 1669454
                    auth = base64.b64encode("%s:%s" % (self.proxy_user,
    ...
    def __createHeaders(self, defaultheaders):
        # BZ 1669454
        auth = base64.b64encode("%s:%s" % (self.username,
                                              self.password)).strip()
