...
class values(Base):
    ...
    def getStatus(self, **kwargs):
        ...
        doc, headers = self._getProxy().get(url)
        logger.debug("REST Response:\n%s" % ParseHelper.toXml(doc))
        status_values = list(value.get_name() for value in doc.get_value())
        return status_values

    def getInternalStatus(self, **kwargs):
        '''
        Its not possible to get a list of internal status values from Strata,
        so just pretend to (to keep the API consistent)
        '''
