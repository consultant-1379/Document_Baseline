    ...
    try:
        ...
        if fileChunk:
            while fh.tell() < fileSize:
                chunk_end = fh.tell() + fileChunk['size']
                ...
                resp = _ftp_storbinary_chunk(ftp, 'STOR %s' % chunkName, fh, chunk_end)
        ...
        # BZ 1547763
        raise Exception("Could not upload file to dropbox: {}". format(e))
    finally:
        # BZ 1547763
        if timer: timer.stop()
        if fh: fh.close()
...
def _ftp_storbinary_chunk(ftpobj, cmd, fp, chunk_end):
    ...
    while fp.tell() < chunk_end:
        conn.sendall(fp.read(4096))
    conn.close()
