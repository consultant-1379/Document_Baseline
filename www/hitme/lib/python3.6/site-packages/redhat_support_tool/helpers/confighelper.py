...
_prompted_for_default_product_save = False
...
class ConfigHelper(object):
    ...
    def __init__(self):
        ...
        if env_var:
            ...
            msg = _('Info : Using proxy_url as defined in http_proxy environment variable: %s') % env_var
            print msg
            #BZ-1670033
            http_proxy_url = urlparse.urlparse(env_var)
            if http_proxy_url.username and http_proxy_url.password:
                if http_proxy_url.scheme and http_proxy_url.hostname and http_proxy_url.port:
                    proxyurl = http_proxy_url.scheme + "://" + http_proxy_url.hostname + ":" + str(http_proxy_url.port)
                    self.set(option='proxy_url', value=proxyurl)
                    self.set(option='proxy_user', value=http_proxy_url.username)
                    passwd_proxy = self.pw_encode(http_proxy_url.password, http_proxy_url.username)
                    self.set(option='proxy_password', value=passwd_proxy)
            else:
    ...
    def remove_option(self, section='RHHelp', option=None,
        ...
        if self.private_config.has_option(section, option):
            self.private_config.remove_option(section, option)
    ...
    def _load_global_config(self):
        ...
        if len(fileAry) == 0:
            ...
            self.global_config.set('RHHelp', 'debug_repos',
                                   self.DEFAULT_DEBUG_REPOS)
    ...
    def _save_global_config(self):
        ...
        try:
            ...
            print _('ERROR: Unable to open %s for writing. Message: %s') % (
                    self.GLOBAL_CONFIG_FILE, ioe.strerror)

        if configfile:
            try:
                self.global_config.write(configfile)
            except IOError, ioe:
                print _('ERROR: Unable to write to %s. Message: %s') % (
                        self.GLOBAL_CONFIG_FILE, ioe.strerror)
            configfile.close()
    ...
    def _prompt_for_default_product(self, default_product):
        # see if we need to prompt the user to save the default product
        global _prompted_for_default_product_save
        prompt_for_save = not _prompted_for_default_product_save

        if default_product and str(default_product).strip():
            default_product = str(default_product).strip()

        if not default_product:
            raise EmptyValueError(_('The Product cannot be empty.'))

        persist = False
        if prompt_for_save:
            val = self.get_from_prompt(_('Save "%s" as the default Product in %s (y/n): ')
            % (default_product, self.dotfile), 'n')
            _prompted_for_default_product_save = True
            if str(val).strip().lower() == 'y':
                persist = True

        self.set(section='RHHelp', option='default_product',
            value = default_product, persist = persist)
