...
#            VMLinux.Elf64_Long = struct.Struct(endianness + 'Q')
    ...
    def __init__(self, filename):
        ...
        logger.log(logging.DEBUG, "Searching for .rodata section...")
        for i in range(len(sections)):
            if sections[i].sect_name == ro_offset:
                logger.log(logging.DEBUG, ".rodata is segment number {0:d}".format(i))

                # now grab the first 192 bytes of the string starting at byte
                # 32 (RHEL5/6) or 192/256 (RHEL7) of this section...
                found = False
                for offset in (32, 192, 256):
                    vmlinux.seek(sections[i].sect_offset + offset, 0)
                ...
                if not found:
                    msg = _('ERROR: Unable to identify kernel version in %s') % \
                        filename
                    print msg
                    logger.log(logging.ERROR, msg)
                    raise Exception(msg)
