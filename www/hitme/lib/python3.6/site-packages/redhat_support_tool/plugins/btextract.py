...
class BTExtract(InteractivePlugin):
  ...
  % (cls.plugin_name)
    ...
    def get_options(cls):
        ...
        return [Option('-c', '--case', dest='casenumber',
                ...
                Option("-o", "--obfuscate", dest="obfuscate",
                    action='store_true',
                    help=_('Obfuscate the data by running it through '
        ...
        try:
            if self._options['casenumber']:
                ...
                for opt in self._submenu_opts:
                    if opt.function_name in ['_send_to_shadowman', '_opencase']:
                        continue
                    ...
                    if len(filtered_string) > 20000:
                        try:
                            ...
                            lh.run('-c %s --description="%s" %s %s' % (
                                                self._options['casenumber'],
                                                opt.display_text.encode("UTF-8", 'replace'),
    ...
    def _obfuscate(self, text):
        # Send text via soscleaner if necessary
        if self._options['obfuscate']:
            return common.obfuscate_text(text)
        else:
            return text
    ...
    def _mkdumpfile_log_fallback(self):
        try:
            ...
            output = self._obfuscate(open(os.path.join(tempdir, 'dmesglog')).read())
    ...
    def _execute_bt_commands(self):
        ...
        print _('Executing crash commands ...')
        # Always do 'bt -a'
        output = self._obfuscate(self.vmcore.exe_crash_commands('bt -a'))
        ...
        if self._options['exframe']:
            output = self._obfuscate(self.vmcore.exe_crash_commands('bt -e'))
        ...
        if self._options['foreachbt']:
            output = self._obfuscate(self.vmcore.exe_crash_commands('foreach bt'))
        ...
        if self._options['log']:
            output = self._obfuscate(self.vmcore.exe_crash_commands('log'))
        ...
        if self._options['ps']:
            output = self._obfuscate(self.vmcore.exe_crash_commands('ps'))
        ...
        if self._options['files']:
            output = self._obfuscate(self.vmcore.exe_crash_commands('files'))
        ...
        if self._options['cmdfile']:
            try:
                ...
                output = self._obfuscate(self.vmcore.exe_crash_commands(file_contents))
