...
class ListAttachments(InteractivePlugin):
    ...
    end_of_entries = _('No more attachments to display')
    _submenu_opts = None
    _sections = None
    case = None
    aAry = None
        ...
        try:
            for val in self.aAry:
                ...
                doc += '\n\n%s%s%s\n\n' % (Constants.BOLD,
                                           ...
                                           Constants.END)
                disp_opt_text = "%-24s  %-36s  %-40s  %s" % (val.get_fileName(),
                                val.get_uuid(), val.get_description(),
                                common.get_friendly_file_length(val.get_length()))
