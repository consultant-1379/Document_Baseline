...
class AddAttachment(Plugin):
    ...
    def get_epilog(cls):
        ...
        return _('Examples:\n'
                 ...
                 '- %s -c 12345678 \n'
                 '- %s -c 12345678 -g') % \
                 (cls.plugin_name, cls.plugin_name, cls.plugin_name, cls.plugin_name)
    ...
    def get_options(cls):
        ...
        errmsg1 = _("ERROR: can't use -s/--split and -x/--no-split options "
                    "together")
        errmsg2 = _("ERROR: the optional argument to -s/--split must be an "
        ...
        def set_split_size_callback(option, opt_str, value, parser):
            ...
            for arg in parser.rargs:
                ...
                if arg[:1] == "-" and len(arg) > 1 and not floatable(arg):
                    break
                # Save the arg if its floatable (a number) and then stop parsing
                if floatable(arg):
                    value.append(arg)
                break
        ...
        return [Option("-c", "--casenumber", dest="casenumber",
                ...
                Option("-r", "--fromopencase", dest="fromopencase", help=SUPPRESS_HELP,
                       action='store_true'),
...
size: %d (MB)" % max_split_size_mb)),
                ...
                Option("-o", "--obfuscate", dest="obfuscate",
                       action='store_true', default=False,
                ...
                Option("-g", "--generatesos", dest="generatesos",
                       action='store_true', default=False,
                       help=_('Generate sosreport and upload to the case '
                              '(requires root user privileges)')),   #BZ 1418701
    ...
    def _check_file(self):
        if self._options['obfuscate']:
            if os.geteuid() != 0:
                msg = _("ERROR: root user privileges required to obfuscate file")
                print msg
                raise Exception(msg)
        if self._options['generatesos']:
            if os.geteuid() != 0:
                msg = _("ERROR: root user privileges required to generate sosreport")
                print msg
                raise Exception(msg)
        msg = _("ERROR: %s requires a path to a file.") % self.plugin_name
        self.attachment = None
        if self._options['generatesos']:
            print("Generating sosreport...")
            p = subprocess.Popen(['sosreport', '--batch'],
        ...
        if self._args and not self._options['generatesos']:
            ...
            #BZ 1418701
            if os.geteuid() == 0 and not self._options['fromopencase'] :
                if not self._options['generatesos']:
                    if self._options['casenumber']:
                        sys.stdout.write(_(
                            'Would you like Red Hat Support Tool to automatically generate and '
                            'attach a SoS report to %s now? (y/N) ') % (self._options['casenumber']))
                    else:
                        sys.stdout.write(_(
                            'Would you like Red Hat Support Tool to automatically generate and '
                            'attach a SoS report now? (y/N) '))
                    line = raw_input()
        ...
        if self._options['obfuscate']:
            self.attachment = common.obfuscate_file(self.attachment)
    ...
    def non_interactive_action(self):
        ...
        try:
            try:
                ...
                print _("Uploading %s to %s ...    " % (uploadBaseName, uploadloc)),
                sys.stdout.flush()
                if self.split_attachment:
                    ...
                    retVal = api.attachments.add(
                                    ...
                                    useFtp=self.use_ftp)
                    if retVal:
                        print _("\b\b\b\bcompleted successfully.")
                ...
                msg = _("ERROR: Unable to fulfill request with support services API.  "
                        "Reason: %s    " % re.reason)
                print _("failed.\n" + msg)
                logger.error(msg)
